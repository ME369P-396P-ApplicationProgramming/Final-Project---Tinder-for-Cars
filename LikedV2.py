...
def logic(all_cars):
    ...
    if responseY:
        ...
        # st.session_state['submit_button'] = st.button("SubmitX")
        st.session_state.liked_cars.append(show_car)
    ...
    if len(st.session_state.liked_cars) %5 == 0 and len(st.session_state.liked_cars) != 0 :
        ...
        if contN:
            ...
            # print(matched_car)
            st.title("THIS IS YOUR PERFECT MATCH!")
            # st.write(str(matched_car[1])) #car make
            # st.write(str(matched_car[2])) #year 
            # st.write(str(matched_car[3])) #mileage
            # st.write(str(matched_car[5])) #price
